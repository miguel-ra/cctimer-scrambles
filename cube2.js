...
const scramble = (function () {
  ...
  */
  ...
  function calctot() {
    ...
    for (let e = 0; e < 24; e++) tot[posit[e]]++;
  }
  const mov2fc = new Array();
  ...
  function domove(y) {
    ...
    const f = y & 15;
    while (q) {
      for (let i = 0; i < mov2fc[f].length; i += 4) {
        const c = posit[mov2fc[f][i]];
  ...
  */
  ...
  const scalePoint = function (w, h, ptIn) {
    const defaultWidth = border * 2 + width * 4 * cubeSize + gap * 3;
    const defaultHeight = border * 2 + width * 3 * cubeSize + gap * 2;

    const scale = Math.min(w / defaultWidth, h / defaultHeight);

    const x =
      Math.floor(ptIn[0] * scale + (w - defaultWidth * scale) / 2) + 0.5;
    const y =
      Math.floor(ptIn[1] * scale + (h - defaultHeight * scale) / 2) + 0.5;

    return [x, y];
  };

  function drawSquare(r, canvasWidth, canvasHeight, cx, cy, w, fillColor) {
    const arrx = [cx - w, cx - w, cx + w, cx + w];
    const arry = [cy - w, cy + w, cy + w, cy - w];

    let pathString = "";
    for (let i = 0; i < arrx.length; i++) {
      const scaledPoint = scalePoint(canvasWidth, canvasHeight, [
        arrx[i],
        arry[i],
      ]);
      pathString += `${(i === 0 ? "M" : "L") + scaledPoint[0]},${
        scaledPoint[1]
      }`;
    }
    pathString += "z";

    r.path(pathString).attr({ fill: colorGet(fillColor), stroke: defaultColors.black });
  }

  const drawScramble = function (parentElement, state, w, h) {
    initializeDrawing();

    const colorString = "wrgoby"; // UFRLBD

    var r = Raphael(parentElement, "100%", "100%").setViewBox(0, 0, w, h);
    ...
    for (i = 0; i < 3 * size; i++) {
      ...
      for (f = 0; f < 4 * size; f++) {
        if (flat2posit[d] < 0) {
          ...
          const c = colorPerm[ori][state[flat2posit[d]]];
          const col = colorList[colors[c] + 0];
          drawSquare(
            r,
            w,
            h,
            border + width / 2 + f * width + gap * Math.floor(f / 2),
            border + width / 2 + i * width + gap * Math.floor(i / 2),
            width / 2,
            col
          );
