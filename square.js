...
const scramble = (function () {
  ...
  function FullCube_randomCube() {
    ...
    shape = Shape_ShapeIdx[~~randomInt.below(3678)];
    ...
    for (i = 0; i < 24; i++) {
      if (((shape >> i) & 1) == 0) {
        ...
        //corner
        rnd = ~~randomInt.below(n_corner) << 2;
    ...
    for (i = prun; i < obj.Search_maxlen2; ++i) {
      if (
        ...
        for (j = 0; j < i; ++j) {
          FullCube_doMove(
            obj.Search_d,
            obj.Search_move[obj.Search_length1 + j]
          );
  ...
  function drawSq(
    ...
    var r = Raphael(parentElement, "100%", "100%").setViewBox(
      0,
      0,
      width,
      height
